<template>
  <div class="play-bar" :class="{ show: !toggle }">
    <div class="fold" :class="{ turn: toggle }">
      <xing-icon :icon="iconList.ZHEDIE" @click="toggle = !toggle"></xing-icon>
    </div>
    <!--播放进度-->
    ...
    <div class="control-box">
      ...
      <div class="song-ctr">
        <xing-icon class="play-show" :icon="playStateList[playStateIndex]" @click="changePlayState"></xing-icon>
        <!--上一首-->
        <xing-icon class="play-show" :icon="iconList.SHANGYISHOU" @click="prev"></xing-icon>
        <!--播放-->
        <xing-icon :icon="playBtnIcon" @click="togglePlay"></xing-icon>
        <!--下一首-->
        <xing-icon class="play-show" :icon="iconList.XIAYISHOU" @click="next"></xing-icon>
        <!--音量-->
        <el-dropdown class="play-show" trigger="click">
          <xing-icon v-if="volume !== 0" :icon="iconList.YINLIANG"></xing-icon>
          <xing-icon v-else :icon="iconList.JINGYIN"></xing-icon>
          <template #dropdown>
            <el-dropdown-menu>
      ...
      <div class="song-ctr song-edit">
        <!--收藏-->
        <xing-icon class="play-show" :class="{ active: isCollection }"
          :icon="isCollection ? iconList.like : iconList.dislike" @click="changeCollection"></xing-icon>
        <!--下载-->
        <xing-icon class="play-show" :icon="iconList.download" @click="downloadMusic({
            songUrl,
            songName: singerName + '-' + songTitle,
          })
          "></xing-icon>
        <!--歌曲列表-->
        <xing-icon :icon="iconList.LIEBIAO" @click="changeAside"></xing-icon>
      </div>
    </div>
  </div>
</template>
